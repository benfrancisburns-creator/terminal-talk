...
| 2026-04-26 03:00      | TT2 | post-/clear  | **main.js sub-2000 closed.** `app/lib/prune.js` extract @ `ce740e3` — pruneOldFiles + pruneSessionsDir lifted to factory. main.js 2031 → **1999**, file-length ceiling restored 2050 → **2000**. PRUNE LIB unit tests @ `6d9a266` lock the contract (6 it() including error-path coverage). 890 → **904**. Clean working tree post-merge of TT1's `50e6871` (Get-StableClaudePid, walks parent chain — solves the PID instability the registry shows in real-time). Resume scoreboard: 4 fix/refactor commits + 2 coord. #18 + #28 + #29-followup all closed |
