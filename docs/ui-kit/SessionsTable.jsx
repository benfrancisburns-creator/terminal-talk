...
function SessionRow({ shortId, entry, expanded, onToggle, onLabel, onIndexChange, onIncludeChange }) {
  ...
  return (
    <div className="tt-session-block">
      <div className="tt-session-row">
        ...
        <div className="tt-swatch" style={{ background: bg }} />
        ...
        <select className="tt-select" value={entry.index || 0} onChange={(e) => onIndexChange(Number(e.target.value))}>
...
function SessionsTable({ sessions, expanded, onToggle, onUpdate }) {
  const entries = Object.entries(sessions).sort((a, b) => (a[1].index || 0) - (b[1].index || 0));
  if (entries.length === 0) {
    return <div className="tt-sessions-empty">No active Claude Code sessions. Open a Claude Code terminal to see one here.</div>;
  }
  return (
    <div className="tt-sessions-table">
      {entries.map(([shortId, entry]) => (
        <SessionRow key={shortId}
          ...
          onIndexChange={(index) => onUpdate(shortId, { ...entry, index })}
