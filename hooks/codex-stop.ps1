...
catch { Write-Error "codex-stop: common module import failed: $($_.Exception.Message)"; exit 1 }

$payload = Read-CodexHookPayload
$sync = if ($payload) { Sync-CodexHookSession -Payload $payload -Caller 'codex-stop' -UpdateTitle } else { $null }
if ($sync -and $sync.short) {
    Set-CodexWorkingFlag -Short ([string]$sync.short) -Action clear -Caller 'codex-stop'
    [void](Start-CodexPluginSessionCleanup -Sync $sync -Caller 'codex-stop')
}
