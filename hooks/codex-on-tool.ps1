...
catch { Write-Error "codex-on-tool: common module import failed: $($_.Exception.Message)"; exit 1 }

$payload = Read-CodexHookPayload
$short = if ($payload) { Resolve-CodexHookWorkingShort -Payload $payload -Caller 'codex-on-tool' } else { '' }
if ($short) {
    Set-CodexWorkingFlag -Short ([string]$short) -Action mark -Caller 'codex-on-tool'
}
