...
- 2026-04-21 T-1: shipped EX5 at `3819827`. Paul-Tol-muted CB palette applied via body[data-palette-variant="cb"] override block in palette-classes.css. New `playback.palette_variant` config + schema enum + validator rule. Toggle row in Settings > Playback. 1 unit test (schema+validator+tokens parity) + 2 E2E tests. AUDIT-FINAL H3 now Option 1 + Option 2 both shipped. Logic tests 195 → 196; Sonar still A·A·A. **T-1 done with the quick-wins lane. Parked EX3/EX4 (need Ben's design dialogue) and EX6/EX7 (own sessions each, don't parallel). If T-2 still hasn't picked up EX2 that's all that's left for this cycle.**
